...
public record HoeHead(int durability, float miningSpeed, Tier tier, float attack) implements IRepairableToolPart {

    public static HoeHead EMPTY = new HoeHead(0, 0f, SerializableTier.EMPTY, 0f);

    public static final MapCodec<HoeHead> CODEC = RecordCodecBuilder.mapCodec(instance ->
            instance.group(
                    ExtraCodecs.NON_NEGATIVE_INT.optionalFieldOf("durability", 0).forGetter(HoeHead::durability),
                    Codec.FLOAT.optionalFieldOf("speed", 0f).forGetter(HoeHead::miningSpeed),
                    HarvestTiers.TIER_CODEC.fieldOf("tier").forGetter(HoeHead::tier),
                    ExtraCodecs.POSITIVE_FLOAT.optionalFieldOf("attack", 0f).forGetter(HoeHead::attack)
            ).apply(instance, HoeHead::new));

    public static final StreamCodec<FriendlyByteBuf, HoeHead> STREAM_CODEC = StreamCodec.composite(
            ByteBufCodecs.INT, HoeHead::durability,
            ByteBufCodecs.FLOAT, HoeHead::miningSpeed,
            HarvestTiers.TIER_STREAM_CODEC, HoeHead::tier,
            ByteBufCodecs.FLOAT, HoeHead::attack,
            HoeHead::new
    );

    @Override
    public ToolPartType<?> getType() {
        return ModulaToolParts.HOE_HEAD.get();
    }

    @Override
    public void statsTooltip(Holder<ToolMaterial> material, Consumer<Component> tooltip, boolean advanced) {
        tooltip.accept(TooltipUtils.part(material, ModulaToolParts.HOE_HEAD));
