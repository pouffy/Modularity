...
public record ShovelHead(int durability, float miningSpeed, Tier tier, float attack) implements IRepairableToolPart {

    public static ShovelHead EMPTY = new ShovelHead(0, 0f, SerializableTier.EMPTY, 0f);

    public static final MapCodec<ShovelHead> CODEC = RecordCodecBuilder.mapCodec(instance ->
            instance.group(
                    ExtraCodecs.NON_NEGATIVE_INT.optionalFieldOf("durability", 0).forGetter(ShovelHead::durability),
                    Codec.FLOAT.optionalFieldOf("speed", 0f).forGetter(ShovelHead::miningSpeed),
                    HarvestTiers.TIER_CODEC.fieldOf("tier").forGetter(ShovelHead::tier),
                    ExtraCodecs.POSITIVE_FLOAT.optionalFieldOf("attack", 0f).forGetter(ShovelHead::attack)
            ).apply(instance, ShovelHead::new));

    public static final StreamCodec<FriendlyByteBuf, ShovelHead> STREAM_CODEC = StreamCodec.composite(
            ByteBufCodecs.INT, ShovelHead::durability,
            ByteBufCodecs.FLOAT, ShovelHead::miningSpeed,
            HarvestTiers.TIER_STREAM_CODEC, ShovelHead::tier,
            ByteBufCodecs.FLOAT, ShovelHead::attack,
            ShovelHead::new
    );

    @Override
    public ToolPartType<?> getType() {
        return ModulaToolParts.SHOVEL_HEAD.get();
    }

    @Override
    public void statsTooltip(Holder<ToolMaterial> material, Consumer<Component> tooltip, boolean advanced) {
        tooltip.accept(TooltipUtils.part(material, ModulaToolParts.SHOVEL_HEAD));
