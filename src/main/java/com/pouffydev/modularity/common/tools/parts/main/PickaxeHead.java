...
public record PickaxeHead(int durability, float miningSpeed, Tier tier, float attack) implements IRepairableToolPart {

    public static PickaxeHead EMPTY = new PickaxeHead(0, 0f, SerializableTier.EMPTY, 0f);

    public static final MapCodec<PickaxeHead> CODEC = RecordCodecBuilder.mapCodec(instance ->
            instance.group(
                    ExtraCodecs.NON_NEGATIVE_INT.optionalFieldOf("durability", 0).forGetter(PickaxeHead::durability),
                    Codec.FLOAT.optionalFieldOf("speed", 0f).forGetter(PickaxeHead::miningSpeed),
                    HarvestTiers.TIER_CODEC.fieldOf("tier").forGetter(PickaxeHead::tier),
                    ExtraCodecs.POSITIVE_FLOAT.optionalFieldOf("attack", 0f).forGetter(PickaxeHead::attack)
            ).apply(instance, PickaxeHead::new));

    public static final StreamCodec<FriendlyByteBuf, PickaxeHead> STREAM_CODEC = StreamCodec.composite(
            ByteBufCodecs.INT, PickaxeHead::durability,
            ByteBufCodecs.FLOAT, PickaxeHead::miningSpeed,
            HarvestTiers.TIER_STREAM_CODEC, PickaxeHead::tier,
            ByteBufCodecs.FLOAT, PickaxeHead::attack,
            PickaxeHead::new
    );

    @Override
    public ToolPartType<?> getType() {
        return ModulaToolParts.PICKAXE_HEAD.get();
    }

    @Override
    public void statsTooltip(Holder<ToolMaterial> material, Consumer<Component> tooltip, boolean advanced) {
        tooltip.accept(TooltipUtils.part(material, ModulaToolParts.PICKAXE_HEAD));
